...
export default function Signup(){
  ...
  const submit = async (e:React.FormEvent)=>{
    ...
    if(password.length<6){ setError('Senha precisa ter >=6 caracteres'); return; }
    ...
    <div className="card mx-auto" style={{maxWidth:480}}>
      <div className="p-4">
        ...
        <form onSubmit={submit}>
          ...
          <input type="password" className="form-control mb-3" placeholder="Confirme a senha" value={confirm} onChange={e=>setConfirm(e.target.value)} />
          <button className="btn btn-success w-100">Criar conta</button>
        </form>
        <div className="mt-2">Já tem conta? <Link to="/login">Entrar</Link></div>
      </div>
    </div>
  );
}
